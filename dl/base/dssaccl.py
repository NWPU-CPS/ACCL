...
class JupiterDssAcclDataset(Dataset):
    @staticmethod
    def collate(batch):
        queryImages = []
        refImages = []
    ...
    def __init__(self, name, queryDir, referenceDir, matchFile, queryPosFile, referencePosFile, kNeighbors, neighborsRange, imageSize, transformSet, maxEpoch, gridN, adapter):
        self.name = name
        ...
        self.queryDistanceMap = self.enrichQConfusionMap(self.qrDistMat, self.qindexVec)

        self.shuffledQIndexes = self.shuffle(self.qrDistMat.max() - self.qrDistMat, self.getKNeighbors(0), self.getKRange(0))
        log.inf("[{}] dssaccl-dataset initialized with <Q-{}, R-{}, k-{}, K-{}, G-{}>".format(
            self.name,
            len(self.queryImagesMap),
            len(self.referenceImagesMap),
            self.kNeighbors,
            self.neighborsRange,
            self.gridN
        ))
    ...
    def enrichQConfusionMap(self, distMat, qindexVec):
        cmap = {}
        dist4Mat, indexMat = torch.topk(distMat, self.gridN, dim=1, largest=False)
        for qi in range(indexMat.shape[0]):
            qindex = qindexVec[qi]
            rawDist4 = dist4Mat[qi]
            dist4 = torch.exp(-(rawDist4 ** 2 / self.adapter.rDelta)) / (rawDist4 + self.adapter.eps)
            cmap[qindex] = (dist4[0] - dist4[1:].mean()).item()
        return cmap
