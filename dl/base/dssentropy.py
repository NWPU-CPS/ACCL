...
class JupiterDssEntropyDataset(Dataset):
    @staticmethod
    def collate(batch):
        queryImages = []
        refImages = []
    ...
    def __init__(self, name, queryDir, referenceDir, matchFile, queryPosFile, referencePosFile, kNeighbors, neighborsRange, imageSize, transformSet, maxEpoch, gridN, adapter):
        ...
        log.inf("[{}] dssentropy-dataset initialized with <Q-{}, R-{}, k-{}, K-{}, G-{}>".format(
            self.name,
    ...
    def enrichQConfusionMap(self, distMat, qindexVec):
        ...
        for qi in range(indexMat.shape[0]):
            ...
            dist4 = 1 + rawDist4.max() - rawDist4
            dist4 /= dist4.sum()
            dist4 = torch.log(dist4) * dist4
            cmap[qindex] = -dist4.sum().item()
        return cmap
