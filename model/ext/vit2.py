...
class ViT(nn.Module):
    ...
    def createTrainModel(cls, mcfg):
        ...
        model = ViT(img_size=img_size, patch_size = patch_size, num_classes=mcfg.outDim, dim=192,
                    mlp_dim_ratio=2, depth=9, heads=12, dim_head=192//12,
                    stochastic_depth=0.1, is_SPT=mcfg.SPT, is_LSA=mcfg.LSA)
    ...
    def load(self, modelFile):
        ...
        if len(missingKeys) > 0:
            ...
            log.grey("Vit2 model loaded from file: {}".format(modelFile))

    def save(self, modelFile, verbose=False):
        torch.save(self.state_dict(), modelFile)
        if verbose:
            log.inf("Vit2 model state saved at {}".format(modelFile))
