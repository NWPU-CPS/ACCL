...
from misc.misc import DropPath
...
class SwinTransformer(nn.Module):
    ...
    def createTrainModel(cls, mcfg):
        ...
        mlp_ratio = 2
        window_size = 4
        img_size = mcfg.inputShape[0]
        patch_size = 2 if img_size == 32 else 4

        model = cls(img_size=img_size, window_size=window_size, drop_path_rate=0.1,
                    patch_size=patch_size, mlp_ratio=mlp_ratio, depths=depths, num_heads=num_heads, num_classes=mcfg.outDim,
                    is_SPT=mcfg.SPT, is_LSA=mcfg.LSA)
    ...
    def createEvalModel(cls, mcfg, modelFile):
        model = cls.createTrainModel(mcfg)
    ...
    def load(self, modelFile):
        ...
        if len(missingKeys) > 0:
            ...
            log.grey("Swin model loaded from file: {}".format(modelFile))

    def save(self, modelFile, verbose=False):
        torch.save(self.state_dict(), modelFile)
        if verbose:
            log.inf("Swin model state saved at {}".format(modelFile))
